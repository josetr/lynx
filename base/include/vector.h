...
struct VectorPrototype {
  ...
  bool is_static_buffer() const { return is_static_; }
 ...
 protected:
  ...
  BASE_VECTOR_NEVER_INLINE static void* _reallocate_buffer(void* array,
    ...
    const auto extra_bytes_size =
        AlignUp(new_capacity * ExtraBytesPerElement, 8);
  ...
  BASE_VECTOR_INLINE void _set_capacity(size_t value, bool is_static) {
    capacity_ = static_cast<uint32_t>(value);
  ...
  BASE_VECTOR_INLINE void _transfer_from(VectorPrototype& other) {
    ...
    count_ = other.count_;
    capacity_field_ = other.capacity_field_;
  }
  ...
  void _swap(VectorPrototype& other) {
    ...
    std::swap(capacity_field_, other.capacity_field_);
  }
 ...
 private:
  ...
  union {
    uint32_t capacity_field_{0};

    struct {
      uint32_t is_static_ : 1;
      uint32_t capacity_ : 31;
    };
  };
};
